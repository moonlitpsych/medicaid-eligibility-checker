 ...
 * @param {string} x12_271 - Raw X12 271 response data
 * @returns {object} - Comprehensive financial information
 */
function extractFinancialBenefits(x12_271) {
    const copayInfo = {
        // Network Status
        isInNetwork: null,  // true if in-network, false if out-of-network, null if unknown
        networkStatus: null, // 'IN_NETWORK', 'OUT_OF_NETWORK', or null
        ...
        // Psychiatric/Mental Health copays (service type A8)
        // These take priority for behavioral health practices
        psychiatricOutpatientCopay: null,
        psychiatricAvailable: false,
    ...
    for (let i = 0; i < segments.length; i++) {
        ...
        if (segment.startsWith('EB*')) {
            ...
            if ((eligibilityCode === 'B' || insurancePlan === 'B' || insurancePlan === 'A') && monetaryAmount !== null) {
                // Match service types to copay categories
                // Note: For service type 98, need to check MSG segments to distinguish PCP vs specialist vs telemedicine
                if (serviceTypes.includes('98') || serviceTypes.includes('BY')) { // Office Visit
                    // Check the next segments for MSG to determine if PCP, specialist, or telemedicine
                    ...
                    if (nextSegments.includes('TELEMEDICINE')) {
                        ...
                        // Some payers list both in the same MSG segments
                    ...
                    else if (nextSegments.includes('SPECIALIST')) {
                    ...
                    // IMPORTANT: Also set as psychiatric copay for behavioral health practices
                    copayInfo.psychiatricOutpatientCopay = monetaryAmount;
                    copayInfo.psychiatricAvailable = true;
    ...
    const hasAnyFinancialInfo = Object.keys(copayInfo).some(key =>
        key !== 'rawBenefits' && copayInfo[key] !== null
    );
