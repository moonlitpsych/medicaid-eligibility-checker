...
function extractFinancialBenefits(x12_271) {
    ...
    for (let i = 0; i < segments.length; i++) {
        ...
        if (segment.startsWith('EB*')) {
            ...
            const eligibilityCode = parts[1]; // 1=Active, C=Deductible, G=OOP Max, etc.
            const coverageLevel = parts[2]; // IND=Individual, FAM=Family
            const serviceTypes = parts[3] ? parts[3].split('^') : [];

            // Handle two different EB segment formats:
            // Format 1 (Standard): EB*C*IND*30**C1*1250  (amount in position 6)
            // Format 2 (UUHP):     EB*C*IND*30***25*1250*****Y (amount in position 7, network flag in position 12)
            ...
            // Check if position 6 has a value (Format 2)
            if (parts[6] && /^\d+(\.\d+)?$/.test(parts[6])) {
                // Format 2: Amount is in position 7
                timePeriodQualifier = parts[6];
                monetaryAmount = parts[7] ? parseFloat(parts[7]) : null;
                percentage = parts[8] ? parseFloat(parts[8]) : null;
                // Network flag is in position 12 (after several asterisks)
                networkFlag = parts[12] ? parts[12].trim() : null;
            } else {
                // Format 1: Amount is in position 6
                monetaryAmount = parts[6] ? parseFloat(parts[6]) : null;
                percentage = parts[7] ? parseFloat(parts[7]) : null;
                // Network flag might still be present
                networkFlag = parts[12] ? parts[12].trim() : null;
            }
            ...
            // Co-payments (B) and Co-insurance (R)
            if ((insurancePlan === 'B' || insurancePlan === 'A') && monetaryAmount) {
                // Match service types to copay categories
                if (serviceTypes.includes('98') || serviceTypes.includes('BY')) { // Office Visit / Primary Care
